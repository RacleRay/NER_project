...
""" Named entity recognition fine-tuning input utils."""
...
def convert_example_to_feature(example_row):
    ...
    处理word piece token的label，在word piece的第一个词位置保留label，而后面的位置进行pad填充
    """
    (
        example,
        label_map,
    ...
    #
    # type_ids：指示来自哪一句；在训练时学习；作为输入加入embedding vector.
    #           [SEP]是一种不明显的划分，和type_ids一起使得更容易学习到序列的划分。
    #
    # 对分类任务而言，[CLS]作为"sentence vector"的代表。但是只有在模型fine-tuned之后，才是有意义的。
    tokens += [sep_token]
    label_ids += [pad_token_label_id]
    if sep_token_extra:   # roberta uses an extra separator b/w pairs of sentences
        tokens += [sep_token]
        label_ids += [pad_token_label_id]
    segment_ids = [sequence_a_segment_id] * len(tokens)

    if cls_token_at_end:  # XLNet does
        tokens += [cls_token]
        label_ids += [pad_token_label_id]
